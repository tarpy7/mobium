...
<div class="flex h-full flex-col">
	<!-- Chat Header -->
	<div class="flex items-center justify-between border-b border-surface-light/40 px-6 py-3" style="background: linear-gradient(135deg, var(--surface) 0%, color-mix(in srgb, var(--surface) 85%, #e07a5f) 50%, color-mix(in srgb, var(--surface) 90%, #81b29a) 100%);">
		<div class="flex items-center gap-3">
			<div class="flex h-9 w-9 items-center justify-center rounded-xl text-sm font-bold flex-shrink-0
				{activeConversation.type === 'dm' ? 'bg-primary/20 text-primary' : activeConversation.type === 'group_dm' ? 'bg-secondary/20 text-secondary' : 'bg-gradient-to-br from-primary/25 to-accent/25 text-primary'}">
				...
				{:else}
					<svg class="h-4.5 w-4.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14" /></svg>
				{/if}
			</div>
			<div>

<script lang="ts">
	...
	let myRole = $derived(members.find(m => m.isSelf)?.role ?? 'member');
	let isOwner = $derived(myRole === 'owner');
	let isMod = $derived(myRole === 'moderator' || myRole === 'owner');
	...
	function truncatePubkey(pk: string): string {
		return pk.length > 16 ? `${pk.substring(0, 8)}…${pk.substring(pk.length - 8)}` : pk;
	}

	function roleBadge(role: string): { label: string; color: string } {
		if (role === 'owner') return { label: '👑 Owner', color: 'text-yellow-600 dark:text-yellow-400' };
		if (role === 'moderator') return { label: '🛡️ Mod', color: 'text-blue-500' };
		return { label: '', color: '' };
	}

	async function saveNickname(pubkey: string) {
		const nick = nicknameInput.trim();
		if (!nick) { editingNickname = null; return; }
		try {
			await invoke('set_nickname', { pubkey, nickname: nick });
			nicknamesStore.update(m => { const nm = new Map(m); nm.set(pubkey, nick); return nm; });
			members = members.map(m => m.pubkey === pubkey ? { ...m, nickname: nick } : m);
	...
	function startDm(pubkey: string) {
		upsertConversation({ id: pubkey, name: displayName(pubkey), type: 'dm', unreadCount: 0 });
	...
	async function addAsFriend(pubkey: string, name: string | null) {
		...
		} catch (e) { addToast('Failed to add friend', 'error'); }
		actionMenuOpen = null;
	}
	...
	async function promoteToMod(pubkey: string) {
		try {
			await invoke('set_member_role', { channelId, targetPubkey: pubkey, role: 'moderator' });
			members = members.map(m => m.pubkey === pubkey ? { ...m, role: 'moderator' } : m);
			addToast('Promoted to moderator', 'success');
		} catch (e) { addToast('Failed to promote', 'error'); }
		actionMenuOpen = null;
	}

	async function demoteToMember(pubkey: string) {
		try {
			await invoke('set_member_role', { channelId, targetPubkey: pubkey, role: 'member' });
			members = members.map(m => m.pubkey === pubkey ? { ...m, role: 'member' } : m);
			addToast('Demoted to member', 'success');
		} catch (e) { addToast('Failed to demote', 'error'); }
		actionMenuOpen = null;
	}

	async function banMember(pubkey: string) {
		const name = getMemberDisplayName(members.find(m => m.pubkey === pubkey)!);
		try {
			await invoke('ban_user', { channelId, targetPubkey: pubkey, reason: null });
			members = members.filter(m => m.pubkey !== pubkey);
			addToast(`${name} has been banned`, 'success');
		} catch (e) { addToast('Failed to ban user', 'error'); }
		actionMenuOpen = null;
	}
	...
	/** Can the current user moderate this member? */
	function canModerate(member: ChannelMember): boolean {
		if (member.isSelf) return false;
		if (member.role === 'owner') return false;
		if (isOwner) return true;
		if (isMod && member.role === 'member') return true;
		return false;
	}
</script>
...
<div class="flex h-full flex-col">
	...
	<div class="flex items-center justify-between border-b border-surface-light px-4 py-3">
		...
		<div class="flex items-center gap-1">
			<button onclick={loadMembers} class="rounded p-1 text-text-muted hover:text-text transition" title="Refresh">
	...
	<div class="flex-1 overflow-y-auto">
		...
		{:else if members.length === 0}
			<div class="p-4 text-center text-sm text-text-muted">No members found.</div>
		{:else}
			{#each members as member}
				...
				<div class="group relative border-b border-surface-light/30 px-4 py-2.5 hover:bg-surface-light/40 transition">
					<div class="flex items-center gap-2.5">
						<!-- Avatar -->
						<div class="flex h-8 w-8 items-center justify-center rounded-full bg-primary/20 text-sm flex-shrink-0 relative">
							{#if member.isSelf}
						...
						<div class="min-w-0 flex-1">
							{#if editingNickname === member.pubkey}
								<div class="flex items-center gap-1">
									...
									<button onclick={() => { editingNickname = null; }} class="text-xs text-text-muted hover:text-text">✕</button>
								</div>
							{:else}
								<button
									data-action-menu
								...
								<div class="flex items-center gap-1.5">
									...
									{/if}
									<button onclick={() => copyPubkey(member.pubkey)}
										class="text-xs text-text-muted/50 hover:text-text-muted transition truncate"
					...
					{#if actionMenuOpen === member.pubkey && !member.isSelf}
						...
						>
							<!-- Send Message -->
							<button onclick={() => startDm(member.pubkey)}
							...
							{:else}
								<button onclick={() => addAsFriend(member.pubkey, getMemberDisplayName(member))}
									class="flex w-full items-center gap-2.5 px-3 py-2 text-xs text-text hover:bg-surface-light/60 transition text-left">
									<svg class="h-3.5 w-3.5 text-text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							...
							<!-- Start Call -->
							<button onclick={() => startCall(member.pubkey)}
								class="flex w-full items-center gap-2.5 px-3 py-2 text-xs text-text hover:bg-surface-light/60 transition text-left">
								<svg class="h-3.5 w-3.5 text-text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							...
							<!-- Set Nickname -->
							<button onclick={() => { editingNickname = member.pubkey; nicknameInput = getMemberDisplayName(member); actionMenuOpen = null; }}
							...
							<!-- Copy Pubkey -->
							<button onclick={() => { copyPubkey(member.pubkey); actionMenuOpen = null; }}
								class="flex w-full items-center gap-2.5 px-3 py-2 text-xs text-text-muted hover:bg-surface-light/60 transition text-left">
								<svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							...
							{#if canModerate(member)}
								...
								{#if isOwner}
									{#if member.role === 'member'}
										<button onclick={() => promoteToMod(member.pubkey)}
											class="flex w-full items-center gap-2.5 px-3 py-2 text-xs text-blue-500 hover:bg-blue-500/10 transition text-left">
											<span class="text-sm">🛡️</span>
											Promote to Moderator
										</button>
									{:else if member.role === 'moderator'}
										<button onclick={() => demoteToMember(member.pubkey)}
											class="flex w-full items-center gap-2.5 px-3 py-2 text-xs text-text-muted hover:bg-surface-light/60 transition text-left">
								...
								<!-- Ban -->
								<button onclick={() => banMember(member.pubkey)}
									class="flex w-full items-center gap-2.5 px-3 py-2 text-xs text-danger hover:bg-danger/10 transition text-left">
									<svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
